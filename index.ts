...
interface ExpressionParser {
    _globals: Set<string>;
    parse: (expr: string, scope?: ReactiveObject, deps?: Set<string>, unwrapRef?: boolean) => any;
...
interface ComponentOptions {
    template: string;
    style?: string;
    script?: (props: any, utils: any) => any;
...
interface Router {
    ...
    init: () => void;
    _parsePath: (path?: string) => string;  // 修改为可选参数
...
declare global {
    interface Window {
        ...
        reactive: (target: any) => ReactiveObject;
        provide: (key: string | Record<string, any>, value?: any) => void;
        dom: (compName: string, options: ComponentOptions) => any;
...
const _nodeContentMap = new WeakMap<Node, string>();              // 存储文本节点的原始内容
const _depsMap = new WeakMap<object, Dependency>();               // 存储响应式对象的依赖关系
const _activeFns: Function[] = [];                                // 当前活跃的更新函数栈
const _directives = new Map<string, DirectiveHandler>();          // 指令处理器注册表
const _elUpdateFns = new WeakMap<HTMLElement, Function>();        // 元素更新函数映射
const _elDeps = new WeakMap<HTMLElement, Set<string>>();          // 元素依赖映射
const _INTERPOLATION_REGEX = /\{\{([^}]+?)}}/g;                   // 匹配插值表达式
const _VARIABLE_REGEX = /[a-zA-Z_$][\w$]*(?:\.[\w$]+)*/g;         // 匹配变量路径
const _pendingProviders: [string, any][] = [];                    // 存储待处理的作用域提供者
const _componentTemplates = new Map<string, DocumentFragment>();  // 组件模板缓存
const _componentInstances = new WeakMap<HTMLElement, any>();      // 组件实例缓存
const _mountedCallbacks: Function[] = [];                         // 存储 onMounted 回调函数
const _inlineScripts: string[] = [];                              // 存储空src的script标签内的代码
 ...
 */
const _BatchUpdater: BatchUpdater = {
    _queue: new Set<Function>(),    // 更新函数队列
...
class _Dependency implements Dependency {
    ...
    _notificationDisabled: any;

    /**
     * 添加订阅者
     */
    subscribe(fn: Function, variable: string | null = null) {
     ...
     * 通知订阅者
     */
    notify(variable: string | null = null) {
 ...
 */
const _ExpressionParser: ExpressionParser = {
     ...
     * @param {boolean} [unwrapRef=true] - 是否自动解包 ref 对象
     * @returns {*} 解析结果
     */
    parse(expr: string, scope: ReactiveObject = {}, deps: Set<string> = new Set(), unwrapRef: boolean = true): any {
        try {
            ...
            // 根据参数决定是否解包 ref
            return unwrapRef && (result as any)?.__isRef ? (result as any).value : result;
        } catch (e) {
    ...
    // 同步修改 parseText 方法，传递 unwrapRef 参数
    parseText(text: string, scope: ReactiveObject = {}, deps: Set<string> = new Set(), unwrapRef: boolean = true): string {
        return text.replace(_INTERPOLATION_REGEX, (_, expr) =>
            this.parse(expr.trim(), scope, deps, unwrapRef) // 传递解包参数
        );
    }
};
 ...
 */
const _createUpdateFn = (el: HTMLElement, scope: ReactiveObject = {}): Function => {
    // 强制更新标记（应对动态指令/属性场景）
    if (_elUpdateFns.has(el)) {
        const cachedFn = _elUpdateFns.get(el)!;
        if (!(el as any)["__forceUpdate"]) return cachedFn; // 若元素未标记"强制更新"，直接复用缓存函数
        (el as any).__forceUpdate = false; // 标记清除：强制更新后重置，避免后续重复触发
    }
    ...
    const attrMap = {
        directives: [] as Array<{ name: string; value: string }>,  // 存储指令属性
        interpolations: [] as Array<{ name: string; value: string }>  // 存储含插值的普通属性
    };
    ...
    // 缓存更新函数：同时存储attrMap
    (updateFn as any).__attrMap = attrMap;  // 挂载attrMap到函数上，便于后续查看属性分类
    ...
    // 避免重复绑定清理事件
    if (!(el as any)["__updateCleanupBound"]) {
        el.addEventListener("beforeunload", cleanup);
        (el as any).__updateCleanupBound = true;
    }
 ...
 */
const _processTextNode = (node: Text, scope: ReactiveObject) => {
    // 缓存原始文本内容
 ...
 */
const _processElement = (el: HTMLElement | DocumentFragment, scope: ReactiveObject = {}) => {
    // 前置校验：快速访问不存在的属性，提前退出
    if (!el || (el as any)["__processedcessed"]) return;
    (el as any).__processed = true;  // 标记处理状态
    ...
    if (nodeType === Node.ELEMENT_NODE) {
        ...
        const updateFn = _createUpdateFn(el as HTMLElement, scope);
    ...
    if (childNodes && childNodes.length) {
        ...
        for (let i = 0, len = childNodes.length; i < len; i++) {
            ...
            if (node.nodeType === Node.TEXT_NODE && _INTERPOLATION_REGEX.test(node.textContent || '')) _processTextNode(node as Text, scope);
        }
    }

    // 仅当当前节点是元素/文档片段时，才处理子元素
    if ((nodeType === Node.ELEMENT_NODE || nodeType === Node.DOCUMENT_FRAGMENT_NODE) && (el as any).children) {
 ...
 */
const _registerDirective = (name: string, handler: DirectiveHandler) => {
...
// r-if 条件渲染指令
_registerDirective("r-if", (el: HTMLElement, expr: string, scope: ReactiveObject, deps: Set<string>) => {
    // 基础校验：提前拦截无效配置，避免后续报错
    if (!scope || typeof scope !== "object") return void console.warn("[r-if] 作用域无效，条件无法解析");

    // 避免重复处理：标记已处理状态，防止同一元素被多次初始化
    if ((el as any).__ifProcessed) return;
    (el as any).__ifProcessed = true;

    // 缓存关键数据：初始display获取逻辑，避免重复读取DOM样式
    let __ifInitialDisplay = (el as any).__ifInitialDisplay;
    if (__ifInitialDisplay === undefined) {
        ...
        (el as any).__ifInitialDisplay = __ifInitialDisplay === "none" ? "block" : __ifInitialDisplay; // 初始display为"none"时，默认用"block"（避免显隐切换时样式丢失）
    }

    // 状态缓存：减少重复解析和DOM操作
    let prevShow: boolean | null = null; // 缓存上一次条件结果，避免相同结果重复执行
    ...
    const update = () => {
        ...
        el.style.display = show ? (el as any).__ifInitialDisplay : "none";
    };
...
// r-click 事件指令
_registerDirective("r-click", (el: HTMLElement, code: string, scope: ReactiveObject) => {
    // 提前返回检查
    ...
    // 提前检查是否需要处理（避免不必要的操作）
    if ((el as any).__clickEventType === eventType && (el as any).__clickCode === code) return;

    // 清理函数
    const cleanup = () => {
        if ((el as any).__clickHandler) {
            el.removeEventListener((el as any).__clickEventType, (el as any).__clickHandler);
            el.removeEventListener("beforeunload", cleanup);
            delete (el as any).__clickHandler;
            delete (el as any).__clickFn;
            delete (el as any).__clickEventType;
            delete (el as any).__clickCode;
        }
    };
    ...
    try {
        ...
        (el as any).__clickFn = clickFn;
        (el as any).__clickCode = code; // 缓存代码用于比较
    } catch (err) {
    ...
    // 绑定事件
    (el as any).__clickHandler = eventHandler;
    (el as any).__clickEventType = eventType;
...
// r-for 循环指令
_registerDirective("r-for", (el: HTMLElement, expr: string, scope: ReactiveObject, deps: Set<string>) => {
    // 基础校验：提前拦截无效配置
    if (expr.trim() === "") return void console.warn("[r-for] 循环表达式不能为空，请检查配置（如 r-for=\"list.length\"）");
    if (!scope || typeof scope !== "object") return void console.warn("[r-for] 作用域无效，循环次数无法解析");

    if ((el as any).__forProcessed) return;
    (el as any).__forProcessed = true;
    ...
    let prevKeys = new Set<number>(); // 记录上一次的索引集合，用于清理过期节点
    ...
    const update = () => {
        ...
        currKeys.forEach(key => {
            ...
            if (cacheEntry) {
                ...
                nodes.forEach(node => {
                    (node as any).__processed = false;
                    if ((node as any).children) Array.from((node as any).children).forEach((child: any) => (child as any).__processed = false);
    ...
    const cleanFor = () => {
        nodeCache.clear();
        prevKeys.clear();
        (el as any).__forProcessed = false;
    ...
    // 依赖更新：变量变化时重新渲染
    const elDeps = _elDeps.get(el) || new Set<string>();
    elDeps.forEach(varName => _depsMap.get(scope)?.subscribe(update, varName));
});


// r-arr 数据循环指令
_registerDirective("r-arr", (el: HTMLElement, expr: string, scope: ReactiveObject, deps: Set<string>) => {
    // 基础校验
    if (expr.trim() === "") return void console.warn("[r-arr] 数组表达式不能为空（如 r-arr=\"list\"）");
    if (!scope || typeof scope !== "object") return void console.warn("[r-arr] 作用域无效，无法解析数组");

    if ((el as any).__arrProcessed) return;
    (el as any).__arrProcessed = true;
    ...
    // 节点缓存（高效复用）
    const nodeCache = new Map<string, { nodes: Node[]; itemScope: ReactiveObject }>(); // key: 唯一键, value: { nodes: Node[], scope: 响应式作用域 }
    let prevKeySet = new Set<string>(); // 上一次的唯一键集合（用于快速对比）
        ...
        currKeys.forEach((currKey, index) => {
            ...
            if (cached) {
                ...
                nodes.forEach(node => {
                    ...
                    (cloned as any).__processed = false; // 重置处理标记，重新收集依赖
                    if ((cloned as any).children) Array.from((cloned as any).children).forEach((child: any) => (child as any).__processed = false);
    ...
    const cleanArr = () => {
        nodeCache.clear();
        prevKeySet.clear();
        (el as any).__arrProcessed = false;
...
// r-api 请求指令
_registerDirective("r-api", async (el: HTMLElement, urlExpr: string, scope: ReactiveObject, deps: Set<string>) => {
    if ((el as any).__apiProcessed) return;
    (el as any).__apiProcessed = true;

    // 核心状态
    let isRequestDone = false;
    let currentData: any = null;
    let lastRenderedData: any = null;
    ...
    // 创建基础作用域
    const createBaseScope = (data: any) => {
        const baseScope: Record<string, any> = {...scope, _aw: isRequestDone};
        if (config.arrKey) baseScope[config.arrKey] = data;
        return window.reactive(baseScope);
    };

    // 渲染数组数据
    const renderArray = (data: any[], fragment: DocumentFragment) => {
        data.forEach((item, index) => {
            ...
            if (cachedNode) {
                ...
                (cachedNode.scope as any)._aw = isRequestDone;
            } else {
                const tempContainer = document.createElement("div");
    ...
    // 渲染对象数据
    const renderObject = (data: any, fragment: DocumentFragment) => {
    ...
    if (config.refreshSelector) {
        ...
        if (refreshBtn && !(refreshBtn as any).__apiRefreshHandler) {
            (refreshBtn as any).__apiRefreshHandler = () => fetchAndRender();
            refreshBtn.addEventListener("click", (refreshBtn as any).__apiRefreshHandler);

            el.addEventListener("beforeunload", () => {
                refreshBtn.removeEventListener("click", (refreshBtn as any).__apiRefreshHandler);
                (refreshBtn as any).__apiRefreshHandler = null;
            });
        }
    }
...
// r-model 双向绑定指令
_registerDirective("r-model", (el: HTMLElement, path: string, scope: ReactiveObject) => {
    // 解析模型路径并返回访问器
    const resolvePath = (pathStr: string, currentScope: ReactiveObject) => {
        ...
        const get = () => {
            let target: any = currentScope;

            // 遍历除最后一段外的所有路径
            for (let i = 0; i < pathSegments.length - 1; i++) {
                const seg = pathSegments[i];
                if (target[seg]?.__isRef) target = target[seg].value;
                else if (target[seg]?.__isReactive) target = target[seg];
        ...
        const set = (newValue: any) => {
            ...
            if (target[lastSeg]?.__isRef) target[lastSeg].value = newValue;
    ...
    const updateView = () => {
        ...
        let viewValue: any; // 用于存储当前DOM中的值

        // 根据元素类型获取当前视图值
        if ((el as HTMLInputElement).type === "checkbox") viewValue = (el as HTMLInputElement).checked;
        else if ((el as HTMLInputElement).type === "radio") viewValue = (el as HTMLInputElement).checked;
        else viewValue = (el as HTMLInputElement).value;

        // 比较模型值和视图值，判断是否需要更新
        let shouldUpdate: boolean;
        if ((el as HTMLInputElement).type === "checkbox") shouldUpdate = !!modelValue !== viewValue; // 转换为布尔值再比较
        else if ((el as HTMLInputElement).type === "radio") shouldUpdate = ((el as HTMLInputElement).value === modelValue) !== viewValue;
        else shouldUpdate = String(modelValue ?? "") !== viewValue; // 统一转为字符串比较

        // 只有当值确实不同时，才执行DOM更新操作
        if (shouldUpdate) {
            if ((el as HTMLInputElement).type === "checkbox") (el as HTMLInputElement).checked = !!modelValue;
            else if ((el as HTMLInputElement).type === "radio") (el as HTMLInputElement).checked = (el as HTMLInputElement).value === modelValue;
            else if (el.tagName === "SELECT") (el as HTMLSelectElement).value = modelValue;
            else (el as HTMLInputElement).value = modelValue != null ? String(modelValue) : "";
        }
    };

    // 更新模型以反映视图变化
    const updateModel = () => {
        let newValue: any; // 用于存储从DOM中获取的新值

        // 根据元素类型获取新值
        if ((el as HTMLInputElement).type === "checkbox") newValue = (el as HTMLInputElement).checked;
        else if ((el as HTMLInputElement).type === "radio") {
            if (!(el as HTMLInputElement).checked) return; // 只有选中的 radio 才触发更新
            newValue = (el as HTMLInputElement).value;
        } else if (el.tagName === "SELECT") newValue = (el as HTMLSelectElement).value;
        else newValue = (el as HTMLInputElement).type === "number" && !isNaN((el as HTMLInputElement).valueAsNumber) ? (el as HTMLInputElement).valueAsNumber : (el as HTMLInputElement).value;
    ...
    // 根据元素类型确定要监听的事件
    const eventType = el.tagName === "SELECT" ? "change" : ["checkbox", "radio"].includes((el as HTMLInputElement).type) ? "change" : "input";

    // 如果元素上已经存在旧的 r-model 事件处理器，先移除它，防止重复绑定
    if ((el as any).__modelHandler) el.removeEventListener((el as any).__modelEventType || "input", (el as any).__modelHandler);

    // 缓存新的事件处理器和事件类型，方便后续清理
    (el as any).__modelHandler = updateModel;
    (el as any).__modelEventType = eventType;
    ...
    const cleanup = () => {
        if ((el as any).__modelHandler) {
            el.removeEventListener((el as any).__modelEventType, (el as any).__modelHandler);
            (el as any).__modelHandler = null; // 释放引用
        }
...
// r-cp 组件化指令
_registerDirective("r-cp", (el: HTMLElement, compName: string, scope: ReactiveObject, deps: Set<string>) => {
    // 避免重复处理组件实例
    if ((el as any).__cpProcessed) return;
    (el as any).__cpProcessed = true;
...
// r-route 路由指令
_registerDirective("r-route", (el: HTMLElement, pathExpr: string, scope: ReactiveObject, deps: Set<string>) => {
    const path = _ExpressionParser.parse(pathExpr, scope, deps);
    if (!path) return;

    // 清理旧处理器
    if ((el as any)._routeHandler) el.removeEventListener("click", (el as any)._routeHandler);

    // 创建新处理器
    (el as any)._routeHandler = (event: Event) => {
        event.preventDefault();
        if (path && window.router.routes.has(path)) window.router.nav(path);
    };

    el.addEventListener("click", (el as any)._routeHandler);

    // 自动清理
    const cleanup = () => {
        el.removeEventListener("click", (el as any)._routeHandler);
        delete (el as any)._routeHandler;
    };
...
// r 指令
_registerDirective("r", (el: HTMLElement, refExpr: string, scope: ReactiveObject, deps: Set<string>) => {
...
// r-dom 指令
_registerDirective("r-dom", (el: HTMLElement, compName: string, scope: ReactiveObject, deps: Set<string>) => {
    // 避免重复处理
    if ((el as any).__domProcessed) return;
    (el as any).__domProcessed = true;
    ...
    // 查找组件工厂函数
    const findComponentFactory = () => scope[compNameTrimmed] || (window.__rootScope && window.__rootScope[compNameTrimmed]);

    // 渲染组件
    const renderComponent = (ComponentFactory: Function) => {
        // 清理旧实例
    ...
    const cleanup = () => {
        ...
        // 清理实例映射
        _componentInstances.delete(el);
        (el as any).__domProcessed = false;
        el.removeEventListener("beforeunload", cleanup);
    };
...
const _Router: Router = {
    ...
    // 添加路由
    add(path: string, handler: Function, target: string = "view") {
    ...
    // 导航到指定路径
    nav(path: string, replace: boolean = false) {
        setTimeout(() => {
    ...
    // 渲染单个页面
    _renderPage(path: string, targetName: string) {
 ...
 */
window.ref = <T>(initialValue: T): Ref<T> => {
 ...
 */
window.reactive = (target: any): ReactiveObject => {
    ...
    if (target.__isReactive) return (_depsMap.get(target) as any)?.__proxy || target;
    ...
    if (Array.isArray(rawTarget)) {
        ...
        rawTarget.forEach((item, index) => (typeof item === "object" && item !== null && !item.__isReactive) && (rawTarget[index] = window.reactive(item)));
    }

    const proxy = new Proxy(rawTarget, {
        get(targetObj, prop: string | symbol, receiver) {
            ...
            if (_activeFns.length > 0) {
                ...
                else dep.subscribe(activeFn, prop as string); // 普通属性：按属性名订阅
            }
            ...
            if (typeof value === "object" && value !== null && !value.__isReactive) return window.reactive(value);
            return value;
        },

        set(targetObj, prop: string | symbol, value: any, receiver) {
            // 禁止修改内置标记
            ...
            // 新值响应式处理
            const reactiveValue = typeof value === "object" && value !== null ? window.reactive(value) : value;
            const setResult = Reflect.set(targetObj, prop, reactiveValue, receiver);

            // 精准通知依赖
            if (!dep._notificationDisabled) {
                if (Array.isArray(targetObj) && /^\d+$/.test(prop as string)) dep.notify(`index:${String(prop)}`); // 数组索引更新
                else dep.notify(prop as string); // 普通属性更新
            }

            return setResult;
        },

        deleteProperty(targetObj, prop: string | symbol) {
            // 禁止删除内置标记
            if (prop === "__isReactive" || prop === "__raw" || prop === "__isReactiveProxy") {
            ...
            if (hadProp && deleteResult && !dep._notificationDisabled) {
                if (Array.isArray(targetObj) && /^\d+$/.test(prop as string)) dep.notify(`index:${String(prop)}`); // 数组索引删除
                else dep.notify(prop as string); // 普通属性删除
            }
    ...
    (dep as any).__proxy = proxy; // 原始对象存储代理引用（便于重复代理检测时直接返回）
 ...
 */
window.provide = (key: string | Record<string, any>, value: any = null) => {
    if (typeof key !== "object") _pendingProviders.push([key, value]);
    else for (const [k, v] of Object.entries(key)) _pendingProviders.push([k, v]);
};
 ...
 */
window.dom = (compName: string, options: ComponentOptions): Function | object => {
    ...
    const scopedId = Math.random().toString(36).substring(2, 8); // 6位随机字符串
    ...
    // CSS作用域实现：为每个选择器添加 [data-v-xxx] 属性选择器
    const addStyleScopeVueStyle = (css: string, scopedId: string): string => {
        // 移除CSS注释
        ...
        while (i < len) {
            // 处理 @ 规则（媒体查询等）
            if (cleanCss[i] === "@") {
                const atRuleEnd = findMatchingBraceInString(cleanCss, i);
                if (atRuleEnd === -1) break;
                const atRule = cleanCss.substring(i, atRuleEnd + 1);
                result.push(processAtRuleWithScoped(atRule, scopedId));
                i = atRuleEnd + 1;
            } else {
                // 处理普通CSS规则
                ...
                result.push(processCSSRuleVueStyle(rule, scopedId));
                i = ruleEnd + 1;
            }
        }
    ...
    // 处理 @ 规则（如媒体查询）中的 scoped
    const processAtRuleWithScoped = (atRule: string, scopedId: string): string => {
        ...
        while (j < innerLen) {
            if (innerContent[j] === "@") {
                const innerAtRuleEnd = findMatchingBraceInString(innerContent, j);
                if (innerAtRuleEnd === -1) break;
                const innerAtRule = innerContent.substring(j, innerAtRuleEnd + 1);
                processedInner += processAtRuleWithScoped(innerAtRule, scopedId);
                j = innerAtRuleEnd + 1;
            } else {
                const innerRuleEnd = innerContent.indexOf("}", j);
                if (innerRuleEnd === -1) break;
                const innerRule = innerContent.substring(j, innerRuleEnd + 1);
                processedInner += processCSSRuleVueStyle(innerRule, scopedId);
                j = innerRuleEnd + 1;
            }
        }
    ...
    // CSS规则处理
    const processCSSRuleVueStyle = (rule: string, scopedId: string): string => {
    ...
    // 在字符串中查找匹配的大括号
    const findMatchingBraceInString = (str: string, start: number): number => {
        let braceCount = 0;
        let inString = false;
        let stringChar = "";
    ...
    // DOM作用域添加函数 data-v-xxx属性
    const addComponentScopeToDOM = (element: HTMLElement | DocumentFragment, isolationEnabled: boolean = sty) => {
        if (element.nodeType === Node.ELEMENT_NODE) {
            // 根据隔离状态决定是否添加作用域属性
            if (isolationEnabled) (element as HTMLElement).setAttribute(`data-v-${scopedId}`, "");
            if ((element as any).children) Array.from((element as any).children).forEach((child: any) => addComponentScopeToDOM(child, isolationEnabled));
        } else if (element.nodeType === Node.DOCUMENT_FRAGMENT_NODE) Array.from((element as any).children).forEach((child: any) => addComponentScopeToDOM(child, isolationEnabled));
    };
    ...
    // 组件工厂函数
    const componentFactory = (pro: Record<string, any> = {}, isolationOverride?: boolean) => {
        ...
        const componentScope = window.reactive({
            $compName: compName,
            $compId: compId,
            $scopedId: scopedId, // 添加scopedId到作用域
            $pro: resolvedPro,
        ...
        // 绑定方法到组件作用域
        Object.entries(scriptResult).forEach(([key, value]) => (typeof value === "function" && !NON_LIFECYCLE_METHODS.has(key)) && ((componentScope as any)[key] = (value as Function).bind(componentScope)));

        // 渲染函数 - 修改为接受样式隔离参数
        const render = (mountTargetEl: HTMLElement, isolationOverride?: boolean) => {
            ...
            // 收集 refs
            const refElements = (templateClone as DocumentFragment).querySelectorAll("[ref]");
            refElements.forEach(el => {
                const refName = el.getAttribute("ref");
                if (refName && !(componentScope.$refs as any)[refName]) (componentScope.$refs as any)[refName] = el;
            });
            ...
            // 根据隔离状态决定是否添加容器标记（使用scopedId）
            if (finalIsolation) mountTargetEl.setAttribute(`data-v-${scopedId}-container`, "");
    ...
    // 挂载函数
    const mountComponent = (pro: Record<string, any>, name: string | HTMLElement, isolationOverride?: boolean) => {
    ...
    const autoRegisterToRoot = () => {
        ...
        const calculateRegisterName = () => {
            // 这里缺少了 `as` 变量的定义，根据原始代码，应该是从 options 中获取
            const as = (options as any).as;
        ...
        // 注册组件到根作用域
        const registerToRootScope = (finalRegisterName: string, targetCompName: string) => {
            const tryRegister = () => {
                if (window.__rootScope) {
                    ...
                    (window.__rootScope as any)[finalRegisterName] = createComponentFactory(finalRegisterName, targetCompName);
 ...
 */
window.onMounted = function (callback: Function) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 创建根作用域
    const rootScope = window.reactive({});
    window.__rootScope = rootScope; // 暴露根作用域用于全局访问

    // 在处理DOM之前，先注入所有提供的数据
    _pendingProviders.forEach(([key, value]) => (rootScope as any)[key] = value);
    _pendingProviders.length = 0; // 清空队列，防止重复处理

    // 初始处理根元素
    _processElement(appRoot as HTMLElement, rootScope);

    // 设置MutationObserver监听DOM变化
    const observer = new MutationObserver(mutations => {
        const toProcess = new Set<HTMLElement>(); // 指令处理集合

        mutations.forEach(mutation => {
            // 处理新增节点
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === Node.ELEMENT_NODE) toProcess.add(node as HTMLElement);
                else if (node.nodeType === Node.TEXT_NODE && _INTERPOLATION_REGEX.test(node.textContent || '')) if (node.parentNode) toProcess.add(node.parentNode as HTMLElement);
            });
    ...
    // 开始观察DOM变化
    observer.observe(appRoot as Node, {childList: true, subtree: true, attributes: true, attributeFilter: Array.from(_directives.keys())});
