...
const _BatchUpdater = {
    ...
    add(fn) {
        if (typeof fn !== "function")
            return; // 严格类型校验，避免无效函数入队
        this._queue.add(fn); // 入队
        if (!this._isUpdating)
            this._scheduleUpdate(); // 调度执行
    },
    /**
     * 调度更新执行
...
class _Dependency {
    ...
    subscribe(fn, variable = null) {
        ...
        if (variable) {
            if (!this.varSubs.has(variable))
                this.varSubs.set(variable, new Set());
    ...
    notify(variable = null) {
        ...
        for (const fn of targets)
            _BatchUpdater.add(fn); // 使用迭代器直接遍历
    }
}
/**
 * 表达式解析工具
...
const _ExpressionParser = {
     ...
     * @returns {*} 解析结果
     */
    parse(expr, scope = {}, deps = new Set(), unwrapRef = true) {
        try {
            const vars = expr.match(_VARIABLE_REGEX) || [];
            vars.forEach(v => {
                const rootVar = v.split(".")[0];
                if (rootVar && !this._globals.has(rootVar) && !deps.has(rootVar))
                    deps.add(rootVar);
            });
            ...
            // 根据参数决定是否解包 ref
            return unwrapRef && result?.__isRef ? result.value : result;
        }
        catch (e) {
    ...
    parseText(text, scope = {}, deps = new Set(), unwrapRef = true) {
        return text.replace(_INTERPOLATION_REGEX, (_, expr) => this.parse(expr.trim(), scope, deps, unwrapRef) // 传递解包参数
        );
    }
};
/**
...
const _createUpdateFn = (el, scope = {}) => {
    ...
    if (_elUpdateFns.has(el)) {
        ...
        if (!el["__forceUpdate"])
            return cachedFn; // 若元素未标记"强制更新"，直接复用缓存函数
    ...
    if (el.attributes) {
        for (let i = 0; i < el.attributes.length; i++) {
            ...
            if (_directives.has(attrName))
                attrMap.directives.push({ name: attrName, value: attrValue }); // 指令属性：直接加入指令列表
            else if (_INTERPOLATION_REGEX.test(attrValue))
                attrMap.interpolations.push({ name: attrName, value: attrValue }); // 普通插值属性：仅含{{}}的属性才加入列表
        }
    }
    ...
    const updateFn = () => {
        ...
        for (let i = 0; i < attrMap.interpolations.length; i++) {
            ...
            if (el.getAttribute(attrName) !== parsedValue)
...
const _processTextNode = (node, scope) => {
    // 缓存原始文本内容
    if (!_nodeContentMap.has(node))
        _nodeContentMap.set(node, node.textContent || '');
    // 更新函数：解析插值并更新文本内容
    const update = () => {
...
const _processElement = (el, scope = {}) => {
    ...
    if (childNodes && childNodes.length) {
        ...
        for (let i = 0, len = childNodes.length; i < len; i++) {
            ...
            if (node.nodeType === Node.TEXT_NODE && _INTERPOLATION_REGEX.test(node.textContent || ''))
                _processTextNode(node, scope);
        }
    }
    ...
    if ((nodeType === Node.ELEMENT_NODE || nodeType === Node.DOCUMENT_FRAGMENT_NODE) && el.children) {
        ...
        if (childLen) {
            ...
            for (let i = 0; i < childLen; i++)
                _processElement(children[i], scope); // 递归处理子元素
        }
    }
};
/**
...
const _registerDirective = (name, handler) => {
    if (typeof handler !== "function")
        throw new Error(`指令处理器必须是函数: ${name}`);
...
_registerDirective("r-if", (el, expr, scope, deps) => {
    ...
    if (!scope || typeof scope !== "object")
        return void console.warn("[r-if] 作用域无效，条件无法解析");
    ...
    const update = () => {
        ...
        // 结果无变化时跳过DOM操作（减少重排）
        if (show === prevShow && !isFirstRender)
            return;
        // 更新状态和DOM
        prevShow = show;
...
_registerDirective("r-click", (el, code, scope) => {
    // 提前返回检查
    if (!code.trim())
        return void console.warn("[r-click] 事件代码不能为空");
    if (!scope || typeof scope !== "object")
        return void console.warn("[r-click] 作用域无效");
    // 一次性获取所有需要的数据
    const eventType = (() => {
        for (const [attr, type] of Object.entries(EVENT_MAP))
    ...
    // 提前检查是否需要处理（避免不必要的操作）
    if (el.__clickEventType === eventType && el.__clickCode === code)
        return;
    // 清理函数
    const cleanup = () => {
    ...
    const eventHandler = (event) => {
        ...
        if (isKeyboardEvent && keyFilter) {
            ...
            if (e.key.toLowerCase() !== normalizedFilter.toLowerCase() && e.key !== normalizedFilter)
                return;
            // 默认行为阻止逻辑
            if (eventType === "keydown" && e.key === "Enter" && e.target.tagName !== "TEXTAREA")
...
_registerDirective("r-for", (el, expr, scope, deps) => {
    // 基础校验：提前拦截无效配置
    if (expr.trim() === "")
        return void console.warn("[r-for] 循环表达式不能为空，请检查配置（如 r-for=\"list.length\"）");
    if (!scope || typeof scope !== "object")
        return void console.warn("[r-for] 作用域无效，循环次数无法解析");
    if (el.__forProcessed)
        return;
    el.__forProcessed = true;
    // 缓存核心数据
    const itemTemplate = el.innerHTML.trim();
    if (!itemTemplate)
        return void console.warn("[r-for] 循环模板不能为空，请在标签内添加内容");
    const indexKey = el.getAttribute("index") || "index"; // 索引变量名（默认index）
    // 缓存基础属性（排除指令相关属性）
    const baseAttrs = {};
    Array.from(el.attributes).forEach(attr => {
        const attrName = attr.name;
        if (["r-for", "index", "class", "style"].includes(attrName))
            return;
        ...
        currKeys.forEach(key => {
            ...
            if (cacheEntry) {
                ...
                nodes.forEach(node => {
                    ...
                    if (node.children)
                        Array.from(node.children).forEach((child) => child.__processed = false);
                ...
                // 应用基础样式和属性
                if (baseClass)
                    tempContainer.className = baseClass;
                if (baseStyle)
                    tempContainer.style.cssText = baseStyle;
...
_registerDirective("r-arr", (el, expr, scope, deps) => {
    // 基础校验
    if (expr.trim() === "")
        return void console.warn("[r-arr] 数组表达式不能为空（如 r-arr=\"list\"）");
    if (!scope || typeof scope !== "object")
        return void console.warn("[r-arr] 作用域无效，无法解析数组");
    if (el.__arrProcessed)
        return;
    el.__arrProcessed = true;
    // 缓存核心配置（减少重复DOM读取）
    const itemTemplate = el.innerHTML.trim();
    if (!itemTemplate)
        return void console.warn("[r-arr] 循环模板不能为空，请在标签内添加内容");
    const indexKey = el.getAttribute("index") || "index"; // 索引变量名（默认index）
    ...
    Array.from(el.attributes).forEach(attr => {
        ...
        if (["r-arr", "index", "value", "key", "class", "style"].includes(name))
            return;
    ...
    const getUniqueKey = (item, index) => {
        ...
        if (item && typeof item === "object" && item[keyProp] !== undefined)
            return String(item[keyProp]); // 转为字符串，避免数字/字符串键冲突
        ...
        currKeys.forEach((currKey, index) => {
            ...
            if (cached) {
                ...
                nodes.forEach(node => {
                    ...
                    if (cloned.children)
                        Array.from(cloned.children).forEach((child) => child.__processed = false);
...
_registerDirective("r-api", async (el, urlExpr, scope, deps) => {
                ...
                try {
                    ...
                    if (typeof exprResult === "string") {
                        ...
                        if (parsedFromString && typeof parsedFromString === "object")
                            headers = parsedFromString;
                    }
                    else if (exprResult && typeof exprResult === "object")
                        headers = exprResult; // 如果已经是对象，直接使用
                }
                catch (exprError) {
    ...
    const parseDynamicUrl = () => {
        ...
        // 如果包含插值表达式，进行解析
        if (_INTERPOLATION_REGEX.test(urlExpr))
            requestUrl = _ExpressionParser.parseText(urlExpr, scope, urlDeps);
        else {
            // 即使没有插值，也要检查是否是变量引用
    ...
    const renderTemplate = (data) => {
        if (JSON.stringify(data) === JSON.stringify(lastRenderedData))
            return;
        el.textContent = "";
        const fragment = document.createDocumentFragment();
        ...
        if (config.arrKey) {
            ...
            renderArray(data, fragment);
        else if (data && typeof data === "object")
            renderObject(data, fragment);
        el.appendChild(fragment);
...
_registerDirective("r-model", (el, path, scope) => {
    ...
    const resolvePath = (pathStr, currentScope) => {
        ...
        const get = () => {
            ...
            for (let i = 0; i < pathSegments.length - 1; i++) {
                ...
                if (target[seg]?.__isRef)
                    target = target[seg].value;
                else if (target[seg]?.__isReactive)
                    target = target[seg];
                else
                    return undefined;
            }
            ...
            if (target[lastSeg]?.__isRef)
                return target[lastSeg].value;
        ...
        const set = (newValue) => {
            ...
            if (target[lastSeg]?.__isRef)
                target[lastSeg].value = newValue;
            else
                target[lastSeg] = newValue;
        };
        return { get, set };
    };
    ...
    const updateView = () => {
        ...
        let shouldUpdate;
        if (el.type === "checkbox")
            shouldUpdate = !!modelValue !== viewValue; // 转换为布尔值再比较
        else if (el.type === "radio")
            shouldUpdate = (el.value === modelValue) !== viewValue;
        else
            shouldUpdate = String(modelValue ?? "") !== viewValue; // 统一转为字符串比较
        // 只有当值确实不同时，才执行DOM更新操作
        if (shouldUpdate) {
            if (el.type === "checkbox")
                el.checked = !!modelValue;
            else if (el.type === "radio")
                el.checked = el.value === modelValue;
    ...
    const updateModel = () => {
        ...
        if (el.type === "checkbox")
            ...
            if (!el.checked)
                return; // 只有选中的 radio 才触发更新
            newValue = el.value;
        }
        else if (el.tagName === "SELECT")
            newValue = el.value;
        else
            newValue = el.type === "number" && !isNaN(el.valueAsNumber) ? el.valueAsNumber : el.value;
        // 获取旧的模型值进行比较
    ...
    if (el.__modelHandler)
        el.removeEventListener(el.__modelEventType || "input", el.__modelHandler);
...
_registerDirective("r-cp", (el, compName, scope, deps) => {
    ...
    if (!compTemplate)
        return void console.error(`[r-cp] 组件 "${compName}" 未定义，请先通过 <template r-cp="${compName}"> 定义`);
    // 提取组件传入的属性
...
_registerDirective("r-route", (el, pathExpr, scope, deps) => {
    ...
    el._routeHandler = (event) => {
        event.preventDefault();
        if (path && window.router.routes.has(path))
            window.router.nav(path);
    };
    ...
    const updateRef = () => {
        ...
        if (newName !== currentRefName) {
            ...
            if (currentRefName && window.$r[currentRefName] === el)
                delete window.$r[currentRefName];
            // 设置新引用
...
_registerDirective("r-dom", (el, compName, scope, deps) => {
    // 避免重复处理
    if (el.__domProcessed)
        return;
    el.__domProcessed = true;
    const compNameTrimmed = compName.trim();
    // 组件模板存在性校验（提前失败）
    if (!_componentTemplates.has(compNameTrimmed))
        return void console.error(`[r-dom] 组件 "${compNameTrimmed}" 未定义，请先通过 dom("${compNameTrimmed}", {...}) 定义`);
    // 组件实例状态管理
    ...
    const renderComponent = (ComponentFactory) => {
        ...
        if (componentInstance) {
            try {
                if (typeof componentInstance.unmount === "function")
        ...
        try {
            // 支持多种调用方式
            if (ComponentFactory.length >= 2)
                componentInstance = ComponentFactory(props, el);
            else
                componentInstance = ComponentFactory({ props, target: el });
            // 验证组件实例
            if (!componentInstance)
                new Error("组件工厂函数未返回有效实例");
...
const _Router = {
    ...
    nav(path, replace = false) {
        setTimeout(() => {
            if (!this.routes.has(path))
                return void console.warn(`路由不存在: ${path}`);
            try {
                const url = new URL(window.location.href);
    ...
    init() {
        ...
        if (initialPath && this.routes.has(initialPath))
            this._executeRoute(initialPath);
        // 浏览器前进后退
    ...
    return {
        ...
        get value() {
            ...
            if (_activeFns.length > 0)
                dep.subscribe(_activeFns[_activeFns.length - 1]);
            return value;
        },
...
window.reactive = (target) => {
    ...
    if (target.__isReactiveProxy)
        return target;
    if (target.__isReactive)
        return _depsMap.get(target)?.__proxy || target;
            ...
            arrayProxyProto[method] = function (...args) {
                ...
                try {
                    ...
                    if (!dep._notificationDisabled)
                        dep.notify("array:mutate"); // 数组变更标记，便于精准订阅
    ...
    const proxy = new Proxy(rawTarget, {
        get(targetObj, prop, receiver) {
            ...
            if (prop === "__proto__" || prop === "__isReactive" || prop === "__raw" || prop === "__isReactiveProxy")
                return Reflect.get(targetObj, prop, receiver);
            if (_activeFns.length > 0) {
                const activeFn = _activeFns[_activeFns.length - 1];
                // 数组索引：标记为 index:0、index:1 等，支持精准更新
                if (Array.isArray(targetObj) && /^\d+$/.test(String(prop)))
                    dep.subscribe(activeFn, `index:${String(prop)}`);
                else
                    dep.subscribe(activeFn, prop); // 普通属性：按属性名订阅
            }
            // 获取原始值
            const value = Reflect.get(targetObj, prop, receiver);
            // 避免初始化时递归代理所有嵌套对象，提升性能
            if (typeof value === "object" && value !== null && !value.__isReactive)
                return window.reactive(value);
            return value;
        },
        set(targetObj, prop, value, receiver) {
            ...
            // 精准通知依赖
            if (!dep._notificationDisabled) {
                if (Array.isArray(targetObj) && /^\d+$/.test(prop))
                    dep.notify(`index:${String(prop)}`); // 数组索引更新
                else
                    dep.notify(prop); // 普通属性更新
            }
            return setResult;
        },
        deleteProperty(targetObj, prop) {
            ...
            if (hadProp && deleteResult && !dep._notificationDisabled) {
                if (Array.isArray(targetObj) && /^\d+$/.test(prop))
                    dep.notify(`index:${String(prop)}`); // 数组索引删除
                else
                    dep.notify(prop); // 普通属性删除
            }
            return deleteResult;
        }
    });
...
window.provide = (key, value = null) => {
    if (typeof key !== "object")
        _pendingProviders.push([key, value]);
    else
        for (const [k, v] of Object.entries(key))
            _pendingProviders.push([k, v]);
};
/**
 * 组件定义函数
...
window.dom = (compName, options) => {
    if (!options || typeof options !== "object")
        throw new Error("dom() 需传入组件名称和配置对象");
    const { template, style, script, pro: proDefinitions, to, sty = true } = options;
    if (!template)
        console.warn(`组件 "${compName}" 缺少 template`);
    if (!window.__componentResetCache)
        window.__componentResetCache = new Set();
    // 生成唯一的组件ID
    ...
    if (!_componentTemplates.has(compName)) {
        ...
        if (template) {
            ...
            while (tempContainer.firstChild)
                templateFragment.appendChild(tempContainer.firstChild);
        }
        _componentTemplates.set(compName, templateFragment);
    }
    else
        templateFragment = _componentTemplates.get(compName);
    // 样式处理
    ...
    const addStyleScopeVueStyle = (css, scopedId) => {
        // 移除CSS注释
    ...
    const processAtRuleWithScoped = (atRule, scopedId) => {
    ...
    // CSS规则处理
    const processCSSRuleVueStyle = (rule, scopedId) => {
        ...
        if (selectors === ":root" || selectors === "html" || selectors === "body")
        ...
        const scopedSelectors = selectors
            ...
            .map(selector => {
            // 处理深层选择器 >>>、/deep/、::v-deep
            if (selector.includes(">>>") || selector.includes("/deep/") || selector.includes("::v-deep"))
                return processDeepSelector(selector, scopedId);
            // 处理伪类、伪元素
    ...
    // 处理深层选择器（样式穿透）
    const processDeepSelector = (selector, scopedId) => {
        const dataAttr = `[data-v-${scopedId}]`;
        // 处理 >>> 语法
        if (selector.includes(">>>"))
            return selector.replace(/\s*>>>\s*/g, ` ${dataAttr} `);
        // 处理 /deep/ 语法
        if (selector.includes("/deep/"))
            return selector.replace(/\s*\/deep\/\s*/g, ` ${dataAttr} `);
        // 处理 ::v-deep 语法
        if (selector.includes("::v-deep"))
            return selector.replace(/::v-deep\s*/g, `${dataAttr} `);
        return selector;
    };
    // 在字符串中查找匹配的大括号
    const findMatchingBraceInString = (str, start) => {
        let braceCount = 0;
        let inString = false;
        let stringChar = "";
    ...
    const addComponentScopeToDOM = (element, isolationEnabled = sty) => {
        if (element.nodeType === Node.ELEMENT_NODE) {
            // 根据隔离状态决定是否添加作用域属性
            if (isolationEnabled)
                element.setAttribute(`data-v-${scopedId}`, "");
            if (element.children)
    ...
    const componentFactory = (pro = {}, isolationOverride) => {
        ...
        if (proDefinitions)
            Object.assign(resolvedPro, proDefinitions);
        ...
        if (typeof scriptResult.setup === "function") {
            try {
                const setupContext = Object.create(null);
                const setupFunc = scriptResult.setup;
                // 创建私有上下文
                const componentPrivateCtx = Object.create(null);
                ...
                // 合并结果并挂载
                const setupResult = { ...setupContext, ...manualReturn };
        ...
        const render = (mountTargetEl, isolationOverride) => {
            if (!mountTargetEl?.nodeType || mountTargetEl.nodeType !== Node.ELEMENT_NODE)
                throw new Error(`组件 "${compName}" 挂载失败：无效的目标节点`);
            // 确定最终的样式隔离状态
            const finalIsolation = isolationOverride !== undefined ? isolationOverride : sty;
            const fragment = document.createDocumentFragment();
            // 处理样式，传入隔离状态
            ...
            refElements.forEach(el => {
                ...
                if (refName && !componentScope.$refs[refName])
                    componentScope.$refs[refName] = el;
            });
            // 处理模板中的指令和响应式数据
            ...
            if (finalIsolation)
                mountTargetEl.setAttribute(`data-v-${scopedId}-container`, "");
            if (lifecycleHooks.mounted)
                requestAnimationFrame(() => lifecycleHooks.mounted.call(componentScope));
    ...
    const autoRegisterToRoot = () => {
        ...
        const createComponentFactory = (_finalRegisterName, targetCompName) => {
            return (...args) => {
                let props;
                let target;
                let styleIsolation;
        ...
        const registerToRootScope = (finalRegisterName, targetCompName) => {
            const tryRegister = () => {
                if (window.__rootScope) {
                    // 避免重复注册
                    if (window.__rootScope[finalRegisterName])
                        return void console.warn(`[dom] 组件名 "${finalRegisterName}" 已被占用，跳过注册`);
                    window.__rootScope[finalRegisterName] = createComponentFactory(finalRegisterName, targetCompName);
                }
                else
                    setTimeout(tryRegister, 40); // 根作用域未就绪，延迟重试
            };
            // 根据文档状态决定注册时机
...
window.onMounted = function (callback) {
    if (typeof callback === "function")
        _mountedCallbacks.push(callback);
    else
        console.warn("onMounted 只接受函数作为参数。");
};
/**
 * 应用初始化
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll("template[r-cp]").forEach((tplEl) => {
        const templateEl = tplEl;
        const compName = templateEl.getAttribute("r-cp")?.trim();
        if (!compName)
            return; // 组件名不能为空
        // 克隆模板内容（避免原模板被修改），存入全局注册表
        const templateFragment = document.createDocumentFragment();
        Array.from(templateEl.content.childNodes).forEach((node) => templateFragment.appendChild(node.cloneNode(true)));
        _componentTemplates.set(compName, templateFragment);
        ...
        mutations.forEach(mutation => {
            ...
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === Node.ELEMENT_NODE)
                    toProcess.add(node);
                else if (node.nodeType === Node.TEXT_NODE && _INTERPOLATION_REGEX.test(node.textContent || ''))
                    if (node.parentNode)
                        toProcess.add(node.parentNode);
            });
            // 处理指令属性变更
            if (mutation.type === "attributes") {
                const target = mutation.target;
                if (_directives.has(mutation.attributeName || ''))
